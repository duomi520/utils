...
func init() {
	...
	CopyUint32(framePing[0:4], 6)
	CopyUint32(framePong[0:4], 6)
	CopyUint32(frameGoaway[0:4], 6)
	CopyUint16(framePing[4:6], StatusPing16)
	CopyUint16(framePong[4:6], StatusPong16)
	CopyUint16(frameGoaway[4:6], StatusGoaway16)
}
...
func (f Frame) MarshalBinary(marshal func(any) ([]byte, error), alloc func(int) []byte) ([]byte, error) {
	...
	CopyUint32(d[0:4], uint32(lenght))
	CopyUint16(d[4:6], f.Status)
	CopyInt64(d[6:14], f.Seq)
	CopyUint16(d[14:16], ServiceMethodEnd)
	CopyUint16(d[16:18], MetadataEnd)
	copy(d[18:ServiceMethodEnd], StringToBytes(f.ServiceMethod))
...
func (f *Frame) UnmarshalHeader(data []byte) (int, error) {
	...
	f.Status = BytesToUint16(data[4:6])
	f.Seq = BytesToInt64(data[6:14])
	ServiceMethodEnd := BytesToUint16(data[14:16])
	MetadataEnd := BytesToUint16(data[16:18])
...
func GetStatus(data []byte) uint16 {
	if len(data) < FrameMinLenght {
		return StatusUnknown16
	}
	return BytesToUint16(data[4:6])
}
