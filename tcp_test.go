...
func TestTCPDial(t *testing.T) {
	...
	_, err := TCPDial(ctx, "127.0.0.1:4567", h, logger)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTCPGracefulStop(t *testing.T) {
	...
	go s.Run()
	_, err := TCPDial(context.TODO(), "127.0.0.1:4568", h, logger)
	if err != nil {
		t.Fatal(err)
	}
	_, err = TCPDial(context.TODO(), "127.0.0.1:4568", h, logger)
...
func TestTCPEcho(t *testing.T) {
	...
	c, err := TCPDial(ctx, "127.0.0.1:4568", hc, logger)
	if err != nil {
		t.Fatal(err)
	}
	data := make([]byte, 10)
	CopyUint32(data[0:4], 10)
	for i := 0; i < loop; i++ {
		CopyUint32(data[6:10], uint32(i))
		c.Send(data)
	}
	<-stop
	time.Sleep(250 * time.Millisecond)
